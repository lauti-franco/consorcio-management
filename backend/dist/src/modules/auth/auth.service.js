...
let AuthService = class AuthService {
    ...
    async login(loginDto) {
        ...
        if (!user.isActive) {
            throw new common_1.UnauthorizedException('La cuenta estÃ¡ desactivada');
        }
    ...
    async validateUser(payload) {
        ...
        if (!user || !user.isActive) {
            throw new common_1.UnauthorizedException('Usuario no vÃ¡lido o inactivo');
        }
        return user;
    }
    ...
    async changePassword(userId, currentPassword, newPassword) {
        ...
        if (!isCurrentPasswordValid) {
            throw new common_1.BadRequestException('ContraseÃ±a actual incorrecta');
        }
        await this.prisma.user.update({
            where: { id: userId },
            data: {
