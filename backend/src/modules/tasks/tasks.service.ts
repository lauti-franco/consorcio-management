...
export class TasksService {
  ...
  async create(createTaskDto: CreateTaskDto & { tenantId: string }, userId: string) {
    ...
    const property = await this.prisma.property.findFirst({
      where: { 
        id: createTaskDto.propertyId, // CAMBIADO: buildingId â†’ propertyId
    ...
    return this.prisma.task.create({
      data: {
        ...
        propertyId: createTaskDto.propertyId, // CAMBIADO: buildingId â†’ propertyId
  ...
  private async verifyTaskAccess(task: any, userId: string, userRole: UserRole, tenantId: string) {
    if (userRole === UserRole.ADMIN) {
      // Verificar que el admin es dueÃ±o de la propiedad
      const property = await this.prisma.property.findFirst({
        where: {
          id: task.propertyId, // CAMBIADO: buildingId â†’ propertyId
          ownerId: userId,
          tenantId: tenantId
        }
      });
    ...
    if (userRole === UserRole.RESIDENT) {
      ...
      const userPropertyAccess = await this.prisma.unit.findFirst({
        where: {
          managerId: userId,
          propertyId: task.propertyId, // CAMBIADO: buildingId â†’ propertyId
          tenantId: tenantId
        },
      });
