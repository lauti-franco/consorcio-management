...
export class BuildingsService {
  ...
  async create(createBuildingDto: CreateBuildingDto & { tenantId: string; ownerId: string }) {
    // Verificar que el tenant existe y estÃ¡ activo
    ...
    // Verificar lÃ­mite de propiedades segÃºn suscripciÃ³n
    ...
    if (userSubscription && propertyCount >= userSubscription.maxProperties) {
      throw new ForbiddenException(
        `LÃ­mite de propiedades alcanzado. Tu plan permite mÃ¡ximo ${userSubscription.maxProperties} propiedades.`
      );
    }
  ...
  // MÃ©todo para verificar acceso basado en roles de tenant (opcional)
    ...
    const property = await this.prisma.property.findFirst({
      where: {
        ...
        OR: [
          ...
          { tickets: { some: { assignedToId: userId } } }, // TÃ©cnico asignado
        ],
      },
    });
