// src/documents/documents.service.ts - VERSIÃ“N COMPLETAMENTE CORREGIDA
...
export class DocumentsService {
  ...
  async findAll(tenantId: string, userId: string, filters?: {
    ...
    // Para residentes, solo mostrar documentos pÃºblicos o con permisos
  ...
  private async checkDocumentAccess(document: any, userId: string, userRole: string): Promise<boolean> {
    // Si es pÃºblico o el uploader
    if (document.isPublic || document.uploadedById === userId) {
      return true;
    }

    // Verificar permisos especÃ­ficos - CORREGIDO
